...
export function setupIpcHandlers(): void {
  // Get chart details (values, schema, etc.)
  ipcMain.handle("chart:getDetails", async (_, { path: chartPath }: { path: string }) => {
    try {
      // Read values.yaml
      const valuesPath = path.join(chartPath, "values.yaml");
      const valuesContent = await fs.readFile(valuesPath, "utf-8");
  ...
  // Save values
  ipcMain.handle("chart:saveValues", async (_, { chartPath, values }: { chartPath: string; values: string }) => {
    try {
      const valuesPath = path.join(chartPath, "values.yaml");
      await fs.writeFile(valuesPath, values, "utf-8");
      return { success: true };
  ...
  ipcMain.handle(
    ...
    async (
      _,
      {
        ...
        valuesYaml,
        chartPath,
      }: { releaseName: string; namespace: string; valuesYaml: string; chartPath: string }
    ) => {
      try {
        // Create a temporary values file
        const tempDir = path.join(app.getPath("temp"), "helm-ui");
  ...
  // Select directory dialog
  ipcMain.handle("dialog:selectDirectory", async (_, options?: Electron.OpenDialogOptions) => {
    const result = await dialog.showOpenDialog({
      properties: ["openDirectory"],
      ...(options || {}),
    });
